...
from transformers.modeling_bert_generation import BertGenerationDecoder, BertGenerationEncoder
...
from transformers.modeling_deberta import DebertaForSequenceClassification, DebertaModel
...
from transformers.modeling_fsmt import FSMTForConditionalGeneration, FSMTModel
from transformers.modeling_funnel import (
    FunnelForMaskedLM,
...
from transformers.modeling_gpt2 import GPT2ForSequenceClassification, GPT2LMHeadModel, GPT2Model
from transformers.modeling_layoutlm import LayoutLMForMaskedLM, LayoutLMForTokenClassification, LayoutLMModel
...
from transformers.modeling_lxmert import LxmertForPreTraining, LxmertModel
...
from transformers.modeling_openai import OpenAIGPTForSequenceClassification, OpenAIGPTLMHeadModel, OpenAIGPTModel
from transformers.modeling_pegasus import PegasusForConditionalGeneration
from transformers.modeling_prophetnet import ProphetNetForCausalLM, ProphetNetForConditionalGeneration, ProphetNetModel
from transformers.modeling_rag import (  # noqa: F401 - need to import all RagModels to be in globals() function
    RagModel,
...
from transformers.modeling_squeezebert import (
...
from transformers.modeling_xlm_prophetnet import (
...
MODEL_FOR_MULTILABEL_SEQUENCE_CLASSIFICATION_MAPPING = OrderedDict(
    [
...
MODEL_WITH_LM_HEAD_MAPPING_2 = OrderedDict(
    [
        (RobertaConfig, RobertaForShuffleRandomThreeWayClassification)
    ]
)
...
class AutoModelForMultiLabelSequenceClassification:
    ...
    @classmethod
    @replace_list_option_in_docstrings(MODEL_FOR_MULTILABEL_SEQUENCE_CLASSIFICATION_MAPPING, use_model_types=False)
    def from_config(cls, config):
        r"""
        Instantiates one of the model classes of the library---with a sequence classification head---from a
        configuration.
        ...
        Examples::
            ...
            >>> model = AutoModelForSequenceClassification.from_config(config)
        """
        if type(config) in MODEL_FOR_MULTILABEL_SEQUENCE_CLASSIFICATION_MAPPING.keys():
            return MODEL_FOR_MULTILABEL_SEQUENCE_CLASSIFICATION_MAPPING[type(config)](config)
        raise ValueError(
            "Unrecognized configuration class {} for this kind of AutoModel: {}.\n"
            "Model type should be one of {}.".format(
                config.__class__,
                cls.__name__,
                ", ".join(c.__name__ for c in MODEL_FOR_MULTILABEL_SEQUENCE_CLASSIFICATION_MAPPING.keys()),
            )
        )

    @classmethod
    @replace_list_option_in_docstrings(MODEL_FOR_MULTILABEL_SEQUENCE_CLASSIFICATION_MAPPING)
    @add_start_docstrings(
        "Instantiate one of the model classes of the library---with a sequence classification head---from a "
        "pretrained model.",
        AUTO_MODEL_PRETRAINED_DOCSTRING,
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if type(config) in MODEL_FOR_MULTILABEL_SEQUENCE_CLASSIFICATION_MAPPING.keys():
            return MODEL_FOR_MULTILABEL_SEQUENCE_CLASSIFICATION_MAPPING[type(config)].from_pretrained(
                pretrained_model_name_or_path, *model_args, config=config, **kwargs
            )
        raise ValueError(
            "Unrecognized configuration class {} for this kind of AutoModel: {}.\n"
